...
/* Mod by Elias */
void SEND_BYTE_SPI(unsigned char data,int PIN_MOSI,int PIN_CLK) 					
	{	
		int i = 0;
		for(i = 0; i < 8; i++) {		
			if(data & 0x80) {				
...
static PyObject *py_bitbang(PyObject *self, PyObject *args) {
    int pin_clk,pin_mosi,pin_cs,size;
    uint8_t *data;
    if (!PyArg_ParseTuple(args, "iiit#", &pin_clk,&pin_mosi,&pin_cs, &data, &size)) return NULL;
    //if (wiringPiDebug) fprintf(stderr, "py_bitbang(%d, [...], %d)\n", device, size);
    //digitalWriteSerialArray(device, data, size);
	sunxi_gpio_output(pin_cs, 0);
	while (size-->0)
	{
		SEND_BYTE_SPI (*data++,pin_mosi,pin_clk) ;
	}
...
PyMethodDef module_methods[] = {
	...
	{"bitbang", py_bitbang, METH_VARARGS, "Bitbangs a Pin."},
